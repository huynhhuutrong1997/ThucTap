{capture name="mainbox"}
    ...
    {capture name="tabsbox"}
        ...
        {$categories_company_id = $product_data.company_id}
        {$allow_save = $product_data|fn_allow_save_object:"product"}

        {if "ULTIMATE"|fn_allowed_for}
            {$categories_company_id = ""}
                ...
                <div id="acc_information" class="collapse in">
                    ...
                    {if $product_type->isFieldAvailable("categories")}
                        ...
                        {if "MULTIVENDOR"|fn_allowed_for && $mode != "add"}
                            {$js_action = "fn_change_vendor_for_product(elm);"}
                        {/if}
                        ...
                        <div class="control-group {$no_hide_input_if_shared_product}" id="product_categories">
                            ...
                            <div class="controls">
                                {include file="pickers/categories/picker.tpl"
                                    hide_input=$product_data.shared_product
                                    company_ids=$runtime.company_id
                                    rnd=$rnd
                                    data_id="categories"
                                    input_name="product_data[category_ids]"
                                    radio_input_name="product_data[main_category]"
                                    main_category=$product_data.main_category
                                    item_ids=$product_data.category_ids|default:$request_category_id
                                    hide_link=true
                    ...
                    {if $product_data.product_type === "\Tygh\Addons\ProductVariations\Product\Manager::PRODUCT_TYPE_VARIATION"|constant}
                        ...
                        {capture name="variation_categories"}
                            {foreach from=$product_data.category_ids|default:$request_category_id item="c_id"}
                                {$category_data = $c_id|fn_get_category_data:$smarty.const.CART_LANGUAGE:"":false:true:false:true}
                        ...
                        <div class="cm-sortable sortable-box" data-ca-sortable-table="images_links" data-ca-sortable-id-name="pair_id" id="additional_images">
                            ...
                            {foreach from=$product_data.image_pairs item=pair name="detailed_images"}
                                ...
                                {if $new_image_position <= $pair.position}
                                    {$new_image_position = $pair.position}
                                {/if}
                            {/foreach}
                        </div>
                    {/if}
