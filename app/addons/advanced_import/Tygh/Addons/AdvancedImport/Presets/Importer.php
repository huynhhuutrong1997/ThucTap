            ...
            if (!empty($modifiers_list['operations'][$parsed['function']])) {
                ...
                if (!is_null($expected_params_qty)) {
                    ...
                    if ($params_qty > $expected_params_qty) {
                        $params = array_slice($params, 0, (int) $expected_params_qty);
                    } elseif ($params_qty < $expected_params_qty) {
                        fn_set_notification('E', __('error'), __('advanced_import.invalid_number_of_parameters_provided', array(
                            '[modifier]' => $modifier,
                        )));
